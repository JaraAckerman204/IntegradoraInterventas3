// ==========================================
// 📄 chiligrin.page.ts - PÁGINA DE CHILIGRIN CON FILTROS
// ==========================================
...
@Component({
  selector: 'app-chiligrin',
  templateUrl: './chiligrin.page.html',
  styleUrls: ['./chiligrin.page.scss'],
  standalone: true,
  imports: [
    IonContent, 
    IonButton,
...
export class ChiligrinPage implements OnInit, AfterViewInit {
  // =============================
  ...
  // =============================
  ngOnInit() {
    console.log('✅ Página de Chiligrin inicializada');
    this.loadProducts();
  ...
  // =============================
  loadProducts() {
    console.log('📄 Iniciando carga de productos Chiligrin...');
    this.loading = true;
    this.products = [];
    ...
    this.productosService.getProductos().subscribe({
      next: (productos) => {
        ...
        // ⭐ FILTRAR SOLO PRODUCTOS DE MARCA CHILIGRIN
        this.products = productos.filter(p => 
          p.marca?.toLowerCase() === 'chiligrin' ||
          p.marca?.toLowerCase().includes('chiligrin')
        );
        
        console.log('🌶️ Productos Chiligrin filtrados:', this.products.length);
        ...
        if (this.products.length === 0) {
          this.mostrarToast('No se encontraron productos Chiligrin disponibles.');
        }
      },
      error: (error) => {
        clearTimeout(timeoutId);
      ...
      complete: () => {
        clearTimeout(timeoutId);
        console.log('🏁 Carga de productos Chiligrin completada');
      }
    });
  }
  ...
  extractFilters() {
    // Extraer categorías únicas de productos Chiligrin
    ...
    console.log('🎨 Filtros extraídos para Chiligrin:', {
  ...
  applyFilters() {
    ...
    console.log('📊 Productos Chiligrin filtrados:', this.filteredProducts.length);
  ...
  addToCartFromModal() {
    ...
    console.log('🛒 Agregando producto Chiligrin al carrito:', {
