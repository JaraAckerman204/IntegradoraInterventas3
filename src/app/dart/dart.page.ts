// ==========================================
// 📄 dart.page.ts - PÁGINA DE dart CON FILTROS
// ==========================================
...
export class DartPage implements OnInit, AfterViewInit {
  // =============================
  ...
  // =============================
  ngOnInit() {
    console.log('✅ Página de dart inicializada');
    this.loadProducts();
  ...
  // =============================
  loadProducts() {
    console.log('📄 Iniciando carga de productos dart...');
    this.loading = true;
    this.products = [];
    ...
    this.productosService.getProductos().subscribe({
      next: (productos) => {
        ...
        // ⭐ FILTRAR SOLO PRODUCTOS DE MARCA dart
        this.products = productos.filter(p => 
          p.marca?.toLowerCase() === 'dart' ||
          p.marca?.toLowerCase().includes('dart')
        );
        
        console.log('🏷️ Productos dart filtrados:', this.products.length);
        ...
        if (this.products.length === 0) {
          this.mostrarToast('No se encontraron productos dart disponibles.');
        }
      },
      error: (error) => {
        clearTimeout(timeoutId);
      ...
      complete: () => {
        clearTimeout(timeoutId);
        console.log('🏁 Carga de productos dart completada');
      }
    });
  }
  ...
  extractFilters() {
    // Extraer categorías únicas de productos dart
    ...
    console.log('🎨 Filtros extraídos para dart:', {
  ...
  applyFilters() {
    ...
    console.log('📊 Productos dart filtrados:', this.filteredProducts.length);
  ...
  addToCartFromModal() {
    ...
    console.log('🛒 Agregando producto dart al carrito:', {
