// ==========================================
// 📄 monark.page.ts - PÁGINA DE monark CON FILTROS
// ==========================================
...
export class MonarkPage implements OnInit, AfterViewInit {
  // =============================
  ...
  // =============================
  ngOnInit() {
    console.log('✅ Página de Monark inicializada');
    this.loadProducts();
  ...
  // =============================
  loadProducts() {
    console.log('📄 Iniciando carga de productos Monark...');
    this.loading = true;
    this.products = [];
    ...
    this.productosService.getProductos().subscribe({
      next: (productos) => {
        ...
        // ⭐ FILTRAR SOLO PRODUCTOS DE MARCA Monark
        this.products = productos.filter(p => 
          p.marca?.toLowerCase() === 'monark' ||
          p.marca?.toLowerCase().includes('monark')
        );
        
        console.log('🏷️ Productos Monark filtrados:', this.products.length);
        ...
        if (this.products.length === 0) {
          this.mostrarToast('No se encontraron productos Monark disponibles.');
        }
      },
      error: (error) => {
        clearTimeout(timeoutId);
      ...
      complete: () => {
        clearTimeout(timeoutId);
        console.log('🏁 Carga de productos Monark completada');
      }
    });
  }
  ...
  extractFilters() {
    // Extraer categorías únicas de productos Monark
    ...
    console.log('🎨 Filtros extraídos para Monark:', {
  ...
  applyFilters() {
    ...
    console.log('📊 Productos Monark filtrados:', this.filteredProducts.length);
  ...
  addToCartFromModal() {
    ...
    console.log('🛒 Agregando producto Monark al carrito:', {
