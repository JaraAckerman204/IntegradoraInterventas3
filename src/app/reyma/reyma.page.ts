// ==========================================
// 📄 reyma.page.ts - PÁGINA DE REYMA CON FILTROS
// ==========================================
...
export class ReymaPage implements OnInit, AfterViewInit {
  // =============================
  ...
  // =============================
  ngOnInit() {
    console.log('✅ Página de Reyma inicializada');
    this.loadProducts();
  ...
  // =============================
  loadProducts() {
    console.log('📄 Iniciando carga de productos Reyma...');
    this.loading = true;
    this.products = [];
    ...
    this.productosService.getProductos().subscribe({
      next: (productos) => {
        ...
        // ⭐ FILTRAR SOLO PRODUCTOS DE MARCA REYMA
        this.products = productos.filter(p => 
          p.marca?.toLowerCase() === 'reyma' ||
          p.marca?.toLowerCase().includes('reyma')
        );
        
        console.log('🏷️ Productos Reyma filtrados:', this.products.length);
        ...
        if (this.products.length === 0) {
          this.mostrarToast('No se encontraron productos Reyma disponibles.');
        }
      },
      error: (error) => {
        clearTimeout(timeoutId);
      ...
      complete: () => {
        clearTimeout(timeoutId);
        console.log('🏁 Carga de productos Reyma completada');
      }
    });
  }
  ...
  extractFilters() {
    // Extraer categorías únicas de productos Reyma
    ...
    console.log('🎨 Filtros extraídos para Reyma:', {
  ...
  applyFilters() {
    ...
    console.log('📊 Productos Reyma filtrados:', this.filteredProducts.length);
  ...
  addToCartFromModal() {
    ...
    console.log('🛒 Agregando producto Reyma al carrito:', {
